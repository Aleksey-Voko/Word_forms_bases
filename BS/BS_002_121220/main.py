...
from BS.word_form import TitleWordForm, GroupWordForm
...
def save_kunstkamera_2():
    ...
    save_bs_dicts_to_txt(sorted(kunstkamera_2),
                         'out/Кунсткамера2.txt', encoding='utf-8')

    remaining_to_the_kunstkamera = [x for x in to_the_kunstkamera_list
    ...
    remaining_word_forms_bases = list(filter(
        lambda x: ' '.join(filter(
            None, [x.title_word_form.name, x.title_word_form.idf,
                   ...
                   x.title_word_form.note])).strip()
                  not in to_the_kunstkamera_list,
